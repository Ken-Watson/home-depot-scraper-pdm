...
class ApiSession:
    ...
    def _create_payload(self, updated_index: int, code: str) -> dict:
    ...
    def make_api_request(self, api_url, index, code):
        """Make API request and return the response"""
        headers = self._get_headers(f"{self.url}")
        payload = self._create_payload(index, code)
