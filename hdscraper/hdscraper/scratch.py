# -*- coding: utf-8 -*-
"""
Created on Mon Apr 11 10:16:24 2022
...
def updated_payload(updated_index):
    """Updated JSON payload for API POST request"""
    return {
...
def scrape_api():
    """Scrapes data from each page of API"""
    new_results = True
    ...
    while new_results:
        response = requests.request(
            ...
            url,
            headers=headers,
            data=json.dumps(updated_payload(updated_index)),
        )
        response_info = json.loads(response.text)
        pprint(response_info)

    #     if response_info["data"]["searchModel"] is None:
    #         new_results = False
    #     elif (
    #         response_info["data"]["searchModel"]["searchReport"]["startIndex"]
    #         < response_info["data"]["searchModel"]["searchReport"]["totalProducts"]
    #     ):
    #         for product_info in tqdm(
    #             response_info["data"]["searchModel"]["products"],
    #             desc="Retrieving category data for current page....",
    #         ):

    #             if product_info["identifiers"]["brandName"] is None:
    #                 product_data["Brand"].append("Unknown")
    #             else:
    #                 product_data["Brand"].append(
    #                     product_info["identifiers"]["brandName"]
    #                 )

    #             if product_info["identifiers"] is None:
    #                 product_data["Item ID"].append("Unknown")
    #             else:
    #                 product_data["Item ID"].append(
    #                     product_info["identifiers"]["itemId"]
    #                 )

    #             if product_info["identifiers"] is None:
    #                 product_data["Model"].append("Unknown")
    #             else:
    #                 product_data["Model"].append(
    #                     str(product_info["identifiers"]["modelNumber"])
    #                 )

    #             if product_info["identifiers"] is None:
    #                 product_data["Description"].append("Unknown")
    #             else:
    #                 product_data["Description"].append(
    #                     product_info["identifiers"]["productLabel"]
    #                 )

    #             if product_info["identifiers"] is None:
    #                 product_data["URL"].append("Unknown")
    #             else:
    #                 product_data["URL"].append(
    #                     "https://www.homedepot.com"
    #                     + product_info["identifiers"]["canonicalUrl"]
    #                 )

    #             if product_info["pricing"] is None:
    #                 product_data["Price"].append("Unknown")
    #             else:
    #                 product_data["Price"].append(product_info["pricing"]["value"])

    #             if product_info["info"]["categoryHierarchy"][-1] is None:
    #                 product_data["Category"].append("Unknown")
    #             else:
    #                 product_data["Category"].append(
    #                     product_info["info"]["categoryHierarchy"][-1]
    #                 )
    ...
    # df = pd.DataFrame({key: pd.Series(value) for key, value in product_data.items()})
...
def main():
    """Main function runs the scripts"""
    scrape_api()


if __name__ == "__main__":

    requested_url = "https://www.homedepot.com/b/Building-Materials-Drywall/N-5yc1vZar3d?catStyle=ShowProducts"
    file_date = "03.11.23"
    file_name_category = "drywall_sheets"
    file_name = f"{file_date}_homedepot_api_data_{file_name_category}.csv"
    category_code = "5yc1vZar3d"
